<template>
  <g-modal title="Yangi ko'rsatkich yaratish" width="1000" @close="$emits('cancel')">
    <template v-slot:body>
      <div class="rows gap-24 mb-24">
        <div class="col-24">
          <g-tab-lang v-model="lang"></g-tab-lang>
        </div>
        <div class="col-24">
          <Input v-model="createForm.comment[lang]" placeholder="Test nomi"></Input>
        </div>
        <div class="col-12">
          <Input v-model="createForm.minPoint" type="number" placeholder="Dan"></Input>
        </div>
        <div class="col-12">
          <Input v-model="createForm.maxPoint" type="number" placeholder="Gacha"></Input>
        </div>
      </div>
    </template>
    <template v-slot:footer>
      <div class="flex gap-12 content-end">
        <Button class="main px-32" @click="create">Yaratish</Button>
        <Button class="px-32" @click="$emits('cancel')">Bekor qilish</Button>
      </div>
    </template>
  </g-modal>
</template>

<script setup>
import GModal from '@/components/g/Modal.vue'
import Button from '@/components/g/Button.vue'
import Input from '@/components/g/Input.vue'
import GTabLang from '@/components/g/TabLang.vue'
import { useAdminTestPointStore } from '@/store/admin/test-point'
import {ref, defineEmits} from 'vue'

const $emits = defineEmits({
    'cancel': null, 
    'created': null
})
const $props = defineProps({
  testId: null
})
const testPointStore = useAdminTestPointStore()
const lang = ref('oz')
const createForm = ref({
  comment: {
    "oz": null,
    "ru": null,
  },
  maxPoint: null,
  minPoint: null,
})

const create = async () => {
  try {
    await testPointStore.create({
      ...createForm.value,
      testId: +$props.testId,
      minPoint: +createForm.value.minPoint,
      maxPoint: +createForm.value.maxPoint,
    })
    $emits('created')
  } catch (e) {
    alert(e)
    console.error(e)
  }
}
</script>