      ...
      <table>
        ...
        <tr v-for="(u, idx) in results" :key="u?.id">
          ...
          <td class="flex items-center gap-8">
            <span>{{ u?.uid }}</span>
            <IconCopy @click="copyUid(u?.uid)"></IconCopy>
          </td>
          <td>
            <GSwitch
              v-model="u.status"
              @change="updateStatus(u.id, u.status)"
            ></GSwitch>
          </td>
          <td class="flex items-center gap-8">
            <Button @click="downloadApp(u?.uid)" class="blue">
              <IconDownload></IconDownload>
            </Button>
            <Button
              @click="router.push(`/cabinet/admin/devices/${u?.id}/activities`)"
              class="info"
            >
...
import CreateModal from "@/components/modal/admin/device/Create.vue";
import UpdateModal from "@/components/modal/admin/device/Update.vue";
...
const fetchList = async ({ params }) => {
  try {
    const response = await axios.get("/admin/device/list", { params });
