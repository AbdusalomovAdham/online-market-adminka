<template>
  <div class="page-header flex gap-12 items-center mb-16">
    <div class="title">
      ...
      <h1 class="white">Katigoriya parametr qiymatlari</h1>
    </div>
...
import CreateModal from "@/components/modal/admin/category/category-param-value/Create.vue";
import UpdateModal from "@/components/modal/admin/category/category-param-value/Update.vue";
...
const cols = [
  ...
  { field: "name", title: "Parametr qiymati", sort: false },
  // { field: "category_name", title: "Katigoriya nomi", sort: false },
  { field: "param_name", title: "Parametr turi", sort: false },
...
const fetchList = async ({ params } = {}) => {
  loading.value = true;
  try {
    const response = await api.get("/admin/param-value/list", {
      params,
    });
...
const deleteItem = async (id) => {
  ...
  try {
    await api.delete(`/admin/param-value/delete/${id}`);
    await refetch();
  } catch (e) {
    console.error(e);
  }
